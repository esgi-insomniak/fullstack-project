...
onMounted(async () => {
    ...
    if (me) {
        isAdmin.value = isLoggedIn.value && me.roles.includes('ROLE_ADMIN')
        isOwner.value = isLoggedIn.value && me.roles.includes('ROLE_DEALER')
    }
    console.log(isAdmin.value, isOwner.value)
})
